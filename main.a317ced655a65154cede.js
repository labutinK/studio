...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  if (navLinks.length > 0) {
    navLinks.forEach(function (link) {
      link.addEventListener('click', function (e) {
        ...
        var targetId = this.getAttribute('href'); // Получаем ID блока
        var targetElement = document.querySelector(targetId); // Находим блок

        if (targetElement) {
          // Плавная прокрутка к блоку
          scrollToElementWithHeaderOffset(targetElement);
        }
      });
    });
  }
  ...
  function init() {
    var myMap = new ymaps.Map("map-wrapper", {
      center: [55.7877, 37.61087],
      // Центр карты
      zoom: 17,
      controls: [] // Отключение управления
    });
    var placemark = new ymaps.Placemark([55.787715, 37.611140],
    // Координаты маркера
    {},
    // Параметры метки (можно оставить пустым)
    {
      iconLayout: "default#image" // Стиль иконки
    });
...
/******/ })()
;
//# sourceMappingURL=main.a317ced655a65154cede.js.map